...
public class filterFlights extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if(!"".equals(fb.getDestination())) {
            List flightList = fb.filterFlightByDestination(destination);
        ...
        else if(!"".equals(fb.getAvailableDay())){
            List flightList = fb.filterFlightByAvailableDay(availableDay);
            request.setAttribute("listOfFlights", flightList);
            RequestDispatcher rd = request.getRequestDispatcher("viewFlightsG2.jsp");
            response.setContentType("text/html; charset='UTF-8'");
            rd.forward(request, response);
